...
Last modified 2020-04-20  by f.maire at qut.edu.au
...
def tree_search(problem, frontier):
    ...
    while frontier:
        ...
        if problem.goal_test(node.state):
            #print(f"Goal found!!!!!!!: {node}") #### this should not trigger as it is false (at least on the first turn/node pop...)
            return node
...
def graph_search(problem, frontier):
    ...
    assert isinstance(problem, Problem)
    frontier.append(Node(problem.initial))
    #print(f"Frontier added: {frontier}")
    ...
    while frontier:
        ...
        # print(f"add to explored: {node.state}") #####
